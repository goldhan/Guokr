...
static int kHEIGHT = 90; //height of the alert view
...
+(RKDropdownAlert*)alertView {
    RKDropdownAlert *alert = [[self alloc]initWithFrame:CGRectMake(0, -kHEIGHT, [[UIScreen mainScreen]bounds].size.width, kHEIGHT)];
    return alert;
}
...
-(void)title:(NSString*)title message:(NSString*)message backgroundColor:(UIColor*)backgroundColor textColor:(UIColor*)textColor time:(NSInteger)seconds
{
    ...
    if (message && message.length > 0) {
        ...
        CGRect frame = titleLabel.frame;
        frame.size.height = kHEIGHT-2*Y_BUFFER-STATUS_BAR_HEIGHT;
